...
public final class DraughtsMan extends DraughtsPiece implements Man {
	...
	protected void generateCaptureList(int dx, int dy, Node<DraughtsNodePosition> node) {
		...
		boolean hasForwardCapture = false;
		boolean hasBackCapture = false;
		...
		for (int dx1 = -1; dx1 <= 1; dx1 += 2) {
			...
			if (!isAhead(dstRow))
				if (game.manDontStarBackCaptures()) {
					if (node.getParent() == null)
						continue;
				} else if (!game.canManMakeBackCaptures())
					continue;
		...
		if (game.canManAvoidBackCaptures() && hasBackCapture && !hasForwardCapture)
			for (int dx1 = -1; dx1 <= 1; dx1 += 2) {
				...
				for (int dy1 = -1; dy1 <= 1; dy1 += 2) {
					...
					node.addChild(new DraughtsNodePosition(new BoardPosition(dstRow, dstCol), node.getValue(), false));
				}
			}
	}
	...
	public boolean gotLastRow() {
		...
		if (isWhite() && dstRow == getGame().getSize() - 1)
			return true;
